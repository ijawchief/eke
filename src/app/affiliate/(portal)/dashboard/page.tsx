...
export default async function AffiliateDashboardPage({
  ...
  const [
    ...
    db.from("affiliate").select("username, name").eq("id", affiliateId).single(),
  ...
  const period_paid_kobo = (paidData ?? []).reduce((s: number, r: { amount_kobo: number }) => s + r.amount_kobo, 0);
  ...
  const stats = [
    { label: "Clicks",          value: String(period_clicks ?? 0),   sub: "product page visits",   color: "text-gray-900" },
    { label: "Conversions",     value: String(period_conversions ?? 0), sub: "completed purchases", color: "text-gray-900" },
    { label: "Earned",          value: fmt(period_paid_kobo),         sub: "paid this period",      color: "text-emerald-600" },
    { label: "Pending Balance", value: fmt(pending_kobo),             sub: "awaiting approval",     color: "text-yellow-600" },
  ];
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">
          Welcome back, <span className="font-extrabold">{affiliate?.name ?? username ?? "Affiliate"}</span>
        </h1>
      </div>
        ...
        <div className="relative rounded-2xl overflow-hidden shadow-sm"
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-4 p-5 sm:p-6">
            ...
            {potd.thumbnail_url ? (
              ...
              <img src={potd.thumbnail_url} alt={potd.name} className="w-full sm:w-24 h-36 sm:h-24 object-cover rounded-xl flex-shrink-0" />
            ) : (
            ...
            <div className="flex sm:flex-col gap-2 w-full sm:w-auto">
              ...
              <a href={`${BASE_URL}/p/${potd.slug}`} target="_blank" rel="noopener noreferrer"
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        {top10.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <Link href="/affiliate/links" className="mt-2 inline-block text-emerald-600 text-sm hover:underline">Browse products →</Link>
          </div>
        ) : (
          <>
                ...
                <div key={p.id} className="px-4 py-3.5 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-0.5 flex-wrap">
                      ...
                      <span className="text-xs text-emerald-600 font-semibold">⚡ {p.gravity}</span>
                ...
                <tbody className="divide-y divide-gray-50">
                  {top10.map((p, i) => (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-3.5 font-bold text-emerald-600">{p.gravity}</td>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="sm:hidden divide-y divide-gray-50">
          {(recent_commissions ?? []).length === 0 && <p className="text-gray-400 text-sm text-center py-10">No commissions yet</p>}
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {(recent_commissions ?? []).length === 0 && <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-400">No commissions yet</td></tr>}
              {(recent_commissions ?? []).map((c: { id: string; amount_kobo: number; status: string; created_at: string; product: { name: string } | { name: string }[] | null }) => {
                const name = Array.isArray(c.product) ? (c.product[0]?.name ?? "—") : (c.product?.name ?? "—");
                return (
