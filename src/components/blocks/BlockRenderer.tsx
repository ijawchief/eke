...
function HeroBlock({ data }: { data: D }) {
  return (
    <div className="py-4 px-1">
      {!!data.badge && (
        <span className="inline-block bg-indigo-50 text-indigo-600 text-xs font-bold px-3 py-1 rounded-full mb-3 tracking-wide uppercase">
          {str(data.badge)}
        </span>
      )}
    ...
    <div className="py-4 px-1">
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div
            key={i}
            className={`rounded-xl border transition-all ${open === i ? "border-indigo-200 bg-indigo-50" : "border-gray-100 bg-white"} overflow-hidden`}
          >
            <button
            ...
            >
              ...
              <ChevronDown
                size={15}
                className={`flex-shrink-0 text-gray-400 transition-transform ${open === i ? "rotate-180 text-indigo-500" : ""}`}
              />
            </button>
            {open === i && (
              <div className="px-4 pb-4 text-gray-600 text-xs leading-relaxed border-t border-indigo-100 pt-3">
                {item.a}
              </div>
            )}
          </div>
  ...
  return (
    <div className="py-3 px-1">
      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-5 text-center">
        {!!data.label && (
          <p className="text-indigo-500 text-xs font-semibold uppercase tracking-widest mb-3">{str(data.label)}</p>
        )}
        <div className="flex justify-center gap-2">
          {units.map(([unit, val]) => (
            <div key={unit} className="bg-white rounded-xl px-3 py-2 min-w-[56px] shadow-sm">
              <div className="text-2xl font-extrabold text-indigo-600 tabular-nums">{String(val).padStart(2, "0")}</div>
