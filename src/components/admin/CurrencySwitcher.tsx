...
export function CurrencySwitcher({ current }: { current: Currency }) {
  ...
  const select = (code: string) => {
    document.cookie = `admin_currency=${code}; path=/; max-age=${60 * 60 * 24 * 365}`;
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 bg-white rounded-xl shadow-xl border border-gray-100 z-50 w-64 overflow-hidden">
