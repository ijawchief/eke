...
export async function sendDeliveryEmail(params: {
  ...
  const amount = params.amountKobo
    ? new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 }).format(params.amountKobo / 100)
    : null;

  await getResend().emails.send({
    from: FROM,
    to: params.to,
    subject: `Your order is confirmed — ${params.productName}`,
    html: `
      <div style="background:#FDF6EE;min-height:100vh;padding:40px 16px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
        <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
          ...
          <div style="background:#C04B1E;padding:32px 32px 28px;">
            <p style="margin:0 0 6px;font-size:13px;color:rgba(255,255,255,0.7);letter-spacing:0.05em;text-transform:uppercase;">Veelage</p>
            <h1 style="margin:0;font-size:26px;font-weight:700;color:#fff;line-height:1.2;">Great choice! 🎉</h1>
          </div>

          <div style="padding:32px;">
            <p style="margin:0 0 16px;font-size:16px;color:#2C2C2C;line-height:1.6;">
              Hi ${params.buyerName ?? "there"},
            </p>
            <p style="margin:0 0 24px;font-size:16px;color:#2C2C2C;line-height:1.6;">
            ...
            <table style="width:100%;border-collapse:collapse;margin-bottom:28px;">
              ...
              ${amount ? `
              <tr style="border-bottom:1px solid #F0E8DF;">
                <td style="padding:12px 0;font-size:13px;color:#999;">Amount Paid</td>
                <td style="padding:12px 0;font-size:14px;font-weight:700;color:#C04B1E;">${amount}</td>
              </tr>` : ""}
              <tr>
                <td style="padding:12px 0;font-size:13px;color:#999;">Order ID</td>
                <td style="padding:12px 0;font-size:12px;color:#bbb;font-family:monospace;">${params.orderRef}</td>
              </tr>
            </table>

            <p style="margin:0 0 20px;font-size:15px;color:#2C2C2C;font-weight:600;">Your download or access details are below:</p>
            ...
            <p style="margin:0 0 6px;font-size:12px;color:#ccc;text-align:center;">Button not working? Copy this link:</p>
            <p style="margin:0 0 28px;font-size:11px;color:#ccc;word-break:break-all;text-align:center;">${params.accessLink}</p>

            <p style="margin:0 0 8px;font-size:14px;color:#2C2C2C;line-height:1.7;">
              If you have any questions, simply reply to this email or contact the creator directly.
            </p>
            <p style="margin:0 0 24px;font-size:14px;color:#2C2C2C;line-height:1.7;">
              Thanks for supporting independent creators. Every purchase helps someone turn their knowledge into a business.
            </p>
            <p style="margin:0;font-size:15px;color:#2C2C2C;font-weight:600;">Enjoy!</p>
          </div>
